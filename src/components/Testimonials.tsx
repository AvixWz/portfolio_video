        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {testimonials.map((testimonial, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              whileHover={{ y: -10 }}
              className="bg-gradient-to-br from-white via-slate-50 to-stone-100 dark:from-slate-800 dark:via-neutral-800 dark:to-zinc-800 rounded-3xl p-8 shadow-2xl border-2 border-emerald-100/50 dark:border-teal-800/50 hover:border-emerald-300/70 dark:hover:border-teal-600/70 relative hover:shadow-emerald-300/30 dark:hover:shadow-teal-900/40 transition-all duration-500 hover:scale-105"
            >
              <Quote className="absolute top-6 right-6 text-emerald-500/20 dark:text-teal-500/20" size={48} />
              ...
              <div className="flex mb-4">
                {[...Array(testimonial.rating)].map((_, i) => (
                  <Star key={i} className="text-amber-400 fill-current drop-shadow-lg" size={18} />
                ))}
              </div>
