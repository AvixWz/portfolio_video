...
import { ExternalLink, ListFilter as Filter, Search, Play, Pause } from 'lucide-react';
    ...
    <div className="pt-24 pb-20">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-16">
          ...
          <h1 className="text-6xl font-bold text-gray-900 dark:text-white mb-4">My Projects</h1>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mb-12">
          <div className="flex flex-col lg:flex-row gap-6 items-center justify-between">
            <div className="flex flex-wrap gap-3 items-center">
              ...
              >
                <Filter className="text-emerald-600 dark:text-teal-400" size={18} />
                <span className="text-sm font-semibold text-emerald-600 dark:text-teal-400">Filter:</span>
              </motion.div>
            ...
            <motion.div
              ...
              className="relative"
            >
              <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 text-emerald-600 dark:text-teal-400" size={20} />
              <input
                type="text"
            ...
            {filteredProjects.map((project, index) => (
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2 group-hover:text-emerald-600 dark:group-hover:text-teal-400 transition-colors">{project.title}</h3>
                  <p className="text-gray-600 dark:text-gray-300 mb-4 text-sm leading-relaxed">{project.description}</p>
                  <div className="flex flex-wrap gap-2">
                    {project.tags.map(tag => (
                      <span key={tag} className="px-3 py-1 bg-slate-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 text-xs font-medium rounded-full hover:bg-emerald-100 dark:hover:bg-teal-900/30 hover:text-emerald-600 dark:hover:text-teal-400 transition-colors">{tag}</span>
                    ))}
                  </div>
                </div>
              </motion.div>
        ...
        {filteredProjects.length === 0 && !isLoading && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20">
            ...
            <p className="text-gray-600 dark:text-gray-300 text-lg">Try adjusting your search terms or filters</p>
          </motion.div>
        )}
        ...
        {isLoading && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20">
            ...
            <p className="text-gray-600 dark:text-gray-300 text-lg font-medium">Loading projects...</p>
          </motion.div>
        )}
        ...
        >
          <h2 className="text-4xl font-bold text-gray-900 dark:text-white mb-4">Ready to Start Your Project?</h2>
          <p className="text-xl text-gray-600 dark:text-gray-300 mb-8">Let's collaborate and bring your creative vision to life</p>
          <motion.a
            href="/contact"
